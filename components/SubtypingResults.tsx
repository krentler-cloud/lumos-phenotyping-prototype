...
function ScatterPlot({ patients }: { patients: Patient[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ fontFamily: "monospace" }}>
      {/* Background */}
      <rect width={W} height={H} fill="#080F1F" rx="8" />
      ...
      {/* Threshold labels */}
      <text x={bdnfThreshX + 2} y={PAD.top + 10} fill="#22C55E" fontSize="7" opacity="0.7">
        BDNF=15
      </text>
      <text x={PAD.left + 2} y={il6ThreshY - 3} fill="#EF4444" fontSize="7" opacity="0.7">
        IL-6=4
      </text>

      {/* Axes */}
      <line x1={PAD.left} y1={PAD.top} x2={PAD.left} y2={H - PAD.bottom} stroke="#1E3A5F" strokeWidth="1" />
      <line x1={PAD.left} y1={H - PAD.bottom} x2={W - PAD.right} y2={H - PAD.bottom} stroke="#1E3A5F" strokeWidth="1" />

      {/* Axis ticks + labels */}
      {xTicks.map(v => (
        <g key={v}>
          <line x1={scaleX(v)} y1={H - PAD.bottom} x2={scaleX(v)} y2={H - PAD.bottom + 3} stroke="#4A6580" strokeWidth="1" />
          <text x={scaleX(v)} y={H - PAD.bottom + 12} fill="#4A6580" fontSize="8" textAnchor="middle">{v}</text>
        </g>
      ))}
      {yTicks.map(v => (
        <g key={v}>
          <line x1={PAD.left - 3} y1={scaleY(v)} x2={PAD.left} y2={scaleY(v)} stroke="#4A6580" strokeWidth="1" />
          <text x={PAD.left - 5} y={scaleY(v) + 3} fill="#4A6580" fontSize="8" textAnchor="end">{v}</text>
        </g>
      ))}

      {/* Axis labels */}
      <text x={(PAD.left + W - PAD.right) / 2} y={H - 4} fill="#8BA3C7" fontSize="9" textAnchor="middle">
        Baseline BDNF (ng/mL)
      </text>
      <text
        x={11} y={(PAD.top + H - PAD.bottom) / 2}
        fill="#8BA3C7" fontSize="9" textAnchor="middle"
      ...
      {/* Data points */}
      {patients.map(p => {
        const color = SUBTYPE_COLORS[p.subtype_label] ?? "#8BA3C7";
...
function MadrsChart({ trajectories }: { trajectories: MadrsTrajectory[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ fontFamily: "monospace" }}>
      ...
      {/* Axes */}
      <line x1={PAD.left} y1={PAD.top} x2={PAD.left} y2={H - PAD.bottom} stroke="#1E3A5F" />
      <line x1={PAD.left} y1={H - PAD.bottom} x2={W - PAD.right} y2={H - PAD.bottom} stroke="#1E3A5F" />

      {/* X labels */}
      {timepoints.map((wk, i) => (
        <text key={wk} x={xPositions[i]} y={H - PAD.bottom + 14} fill="#4A6580" fontSize="9" textAnchor="middle">
          {wk === 0 ? "Baseline" : `Wk ${wk}`}
        </text>
      ))}
      ...
      {/* Y labels */}
      {yTicks.map(v => (
        <text key={v} x={PAD.left - 5} y={scaleY(v) + 3} fill="#4A6580" fontSize="8" textAnchor="end">{v}</text>
      ))}
    ...
    <div className="max-w-5xl mx-auto px-8 py-10">
      ...
      <div className="flex items-start justify-between mb-8">
        ...
        <Link
          ...
          className="flex-shrink-0 px-4 py-2 bg-[#4F8EF7] text-white font-semibold text-sm rounded-xl hover:bg-[#3A7AE8] transition-colors"
        >
          Final Report →
        </Link>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-8">
        {[
          { label: "Responders", value: ml.responder_count, color: "#22C55E" },
          { label: "Non-Responders", value: ml.nonresponder_count, color: "#EF4444" },
          { label: "Uncertain", value: ml.uncertain_count, color: "#F59E0B" },
          { label: "Concordance", value: `${ml.concordance_pct}%`, color: "#4F8EF7" },
        ].map(card => (
          <div key={card.label} className="p-4 bg-[#0F1F3D] border border-[#1E3A5F] rounded-xl text-center">
            <p className="text-[10px] uppercase tracking-widest text-[#4A6580] mb-1">{card.label}</p>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        <div className="bg-[#080F1F] border border-[#1E3A5F] rounded-xl p-4">
          <p className="text-[10px] uppercase tracking-widest text-[#4A6580] mb-3">
          ...
          <div className="flex gap-4 mt-3 justify-center flex-wrap">
            {Object.entries(SUBTYPE_LABELS).map(([k, label]) => (
              <div key={k} className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] text-[#8BA3C7]">{label.split(" — ")[0]}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="flex gap-4 mt-3 justify-center flex-wrap">
            {ml.madrs_trajectories.filter(t => t.subtype !== 'Overall').map(t => (
              <div key={t.subtype} className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] text-[#8BA3C7]">{SUBTYPE_LABELS[t.subtype]?.split(" — ")[0]}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Bayesian update */}
      <div className="mb-8">
        <p className="text-[10px] uppercase tracking-widest text-[#4A6580] mb-3">Bayesian Confidence Update</p>
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: "Overall Analysis", prior: bu.overall.prior, posterior: bu.overall.posterior, color: "#4F8EF7" },
            { label: "Responder Hypothesis", prior: bu.responder.prior, posterior: bu.responder.posterior, color: "#22C55E" },
            { label: "Non-Responder Hypothesis", prior: bu.nonresponder.prior, posterior: bu.nonresponder.posterior, color: "#EF4444" },
          ].map(card => {
            const priorPct = Math.round(card.prior * 100);
            const postPct = Math.round(card.posterior * 100);
            const delta = postPct - priorPct;
            return (
              <div key={card.label} className="p-4 bg-[#0F1F3D] border border-[#1E3A5F] rounded-xl">
                <p className="text-[10px] uppercase tracking-widest mb-1" style={{ color: card.color }}>{card.label}</p>
                <div className="flex items-end gap-2 mb-2">
                  <span className="text-[#4A6580] text-sm line-through">{priorPct}%</span>
                  <span className="text-xl font-bold text-[#F0F4FF]">{postPct}%</span>
                ...
                <div className="space-y-1">
                  <div className="h-1 bg-[#1E3A5F] rounded-full overflow-hidden">
                    <div className="h-full rounded-full bg-[#1E3A5F]" style={{ width: `${priorPct}%`, opacity: 0.4 }} />
                  </div>
                  <div className="h-1 bg-[#1E3A5F] rounded-full overflow-hidden">
      ...
      {/* Feature importance */}
      <div className="mb-8">
        <p className="text-[10px] uppercase tracking-widest text-[#4A6580] mb-3">Predictive Feature Importance</p>
        <div className="bg-[#080F1F] border border-[#1E3A5F] rounded-xl p-5 space-y-3">
          {ml.feature_importance.slice(0, 7).map((f, i) => {
            const barColor = f.direction === "positive" ? "#22C55E" : "#EF4444";
            return (
              <div key={f.feature} className="flex items-center gap-3">
                <span className="text-[#4A6580] text-xs w-4 text-right">{i + 1}</span>
                <span className="text-[#D0DCF0] text-xs w-40 flex-shrink-0">{f.label}</span>
                ...
                <span className="text-[10px] text-[#4A6580] w-20">
      ...
      <div className="mb-8">
        <p className="text-[10px] uppercase tracking-widest text-[#4A6580] mb-3">Patient Subtype Summary</p>
            ...
            return (
              <div key={st} className="p-4 border rounded-xl" style={{ borderColor: `${color}30`, background: `${color}08` }}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="font-semibold text-sm text-[#F0F4FF]">{SUBTYPE_LABELS[st]}</span>
                </div>
                <div className="space-y-1.5 text-xs text-[#8BA3C7]">
                  <div className="flex justify-between">
                    <span>N</span>
                    <span className="text-[#F0F4FF] font-semibold">{pts.length}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-[#F0F4FF] font-semibold">{avgMadrsReduction}%</span>
                  </div>
                </div>
              </div>
            );
      ...
      <div className="text-center">
        <Link
          href={`/studies/${studyId}/phase2/report`}
          className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-[#4F8EF7] to-[#A855F7] text-white font-semibold text-sm rounded-xl hover:opacity-90 transition-opacity"
        >
