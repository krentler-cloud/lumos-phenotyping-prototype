export const dynamic = "force-dynamic";

import { createServiceClient } from "@/lib/supabase/server";
import Link from "next/link";

const statusColors: Record<string, string> = {
  complete: "text-status-success bg-status-success/12 border-status-success",
  processing: "text-brand-core bg-brand-core/12 border-brand-core",
  queued: "text-status-warning bg-status-warning/12 border-status-warning",
  error: "text-status-danger bg-status-danger/12 border-status-danger",
};

export default async function AdminPage() {
  const supabase = createServiceClient();

  const [
    { data: docs },
    { count: totalChunks },
    { data: allRuns },
    { count: totalPatients },
  ] = await Promise.all([
    supabase.from("corpus_docs").select("id, title, source_type, chunk_count, status, created_at").order("created_at", { ascending: false }),
    supabase.from("corpus_chunks").select("*", { count: "exact", head: true }),
    supabase.from("runs").select("id, status, phase, created_at, study_id, error_message").order("created_at", { ascending: false }).limit(50),
    supabase.from("clinical_patients").select("*", { count: "exact", head: true }),
  ]);

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const runs = (allRuns as any[]) ?? []
  const goodRuns = runs.filter((r: { status: string }) => r.status !== "error")
  const failedRuns = runs.filter((r: { status: string }) => r.status === "error")

  // Only count good docs in stats
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const readyDocs = (docs as any[] ?? []).filter((d: { status: string }) => d.status === "ready")
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const errorDocs = (docs as any[] ?? []).filter((d: { status: string }) => d.status === "error")

  const runCounts = goodRuns.reduce((acc: Record<string, number>, r: { status: string }) => {
    acc[r.status] = (acc[r.status] ?? 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  return (
    <div className="max-w-7xl mx-auto px-6 py-12 w-full space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-text-heading mb-1">Admin</h1>
        <p className="text-text-muted text-sm">System health and corpus overview</p>
      </div>

      {/* Stats — only good data */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Corpus Docs", value: readyDocs.length, icon: "📄" },
          { label: "Embeddings", value: (totalChunks ?? 0).toLocaleString(), icon: "🧩" },
          { label: "Clinical Patients", value: totalPatients ?? 0, icon: "👤" },
          { label: "Completed Runs", value: goodRuns.filter((r: { status: string }) => r.status === "complete").length, icon: "▶" },
        ].map(s => (
          <div key={s.label} className="bg-bg-surface border border-border-subtle rounded-xl p-5">
            <div className="text-xl mb-2">{s.icon}</div>
            <div className="text-2xl font-bold text-text-heading">{s.value}</div>
            <div className="text-text-muted text-sm">{s.label}</div>
          </div>
        ))}
      </div>

      {/* Run status breakdown */}
      <div className="bg-bg-surface border border-border-subtle rounded-xl p-6">
        <h2 className="text-text-heading font-semibold mb-4">Run Status</h2>
        <div className="flex gap-3 flex-wrap">
          {Object.entries(runCounts).map(([status, count]) => (
            <span key={status} className={`px-3 py-1 rounded-full text-sm border capitalize ${statusColors[status] ?? ""}`}>
              {status}: {count as number}
            </span>
          ))}
          {failedRuns.length > 0 && (
            <span className="px-3 py-1 rounded-full text-sm border text-status-danger bg-status-danger/12 border-status-danger">
              error: {failedRuns.length}
            </span>
          )}
          {runs.length === 0 && <span className="text-text-muted text-sm">No runs yet</span>}
        </div>
      </div>

      {/* Corpus docs — ready only */}
      <div className="bg-bg-surface border border-border-subtle rounded-xl p-6">
        <h2 className="text-text-heading font-semibold mb-4">Corpus Documents</h2>
        {!readyDocs.length ? (
          <p className="text-text-muted text-sm">No documents ingested yet. <Link href="/corpus" className="text-brand-core hover:underline">Go to Corpus →</Link></p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border-subtle text-text-muted text-xs uppercase">
                  <th className="text-left py-2 pr-4">Title</th>
                  <th className="text-left py-2 pr-4">Type</th>
                  <th className="text-left py-2 pr-4">Chunks</th>
                  <th className="text-left py-2">Ingested</th>
                </tr>
              </thead>
              <tbody>
                {readyDocs.map((doc: { id: string; title: string; source_type: string; chunk_count: number; created_at: string }) => (
                  <tr key={doc.id} className="border-b border-border-subtle last:border-0">
                    <td className="py-3 pr-4 text-text-heading">{doc.title}</td>
                    <td className="py-3 pr-4 text-text-muted capitalize">{doc.source_type.replace("_", " ")}</td>
                    <td className="py-3 pr-4 text-brand-core font-mono">{doc.chunk_count ?? "—"}</td>
                    <td className="py-3 text-text-muted text-xs">{new Date(doc.created_at).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        )}
      </div>

      {/* Recent good runs */}
      <div className="bg-bg-surface border border-border-subtle rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-text-heading font-semibold">Recent Runs</h2>
          <Link href="/runs" className="text-brand-core text-sm hover:underline">View all →</Link>
        </div>
        {!goodRuns.length ? (
          <p className="text-text-muted text-sm">No runs yet.</p>
        ) : (
          <div className="space-y-2">
            {goodRuns.map((run: { id: string; status: string; phase: string; created_at: string; study_id: string }) => (
              <div key={run.id} className="flex items-center justify-between text-sm py-2 border-b border-border-subtle last:border-0">
                <div className="flex items-center gap-3">
                  <span className={`px-2 py-0.5 rounded-full text-xs border capitalize ${statusColors[run.status] ?? ""}`}>{run.status}</span>
                  <span className="text-text-muted text-xs capitalize">{run.phase ?? "—"}</span>
                  <span className="text-text-secondary font-mono text-xs">{run.id.slice(0, 8)}</span>
                </div>
                <div className="flex items-center gap-4">
                  {run.status === "complete" && run.study_id && (
                    <Link
                      href={run.phase === "clinical" ? `/studies/${run.study_id}/phase2/report` : `/studies/${run.study_id}/phase1/report`}
                      className="text-brand-core text-xs hover:underline"
                    >
                      View report →
                    </Link>
                  )}
                  <span className="text-text-muted text-xs">{new Date(run.created_at).toLocaleString()}</span>
                </div>
              </div>
            ))}
          </div>
        )}
      </div>

      {/* Failed runs — collapsed */}
      {(failedRuns.length > 0 || errorDocs.length > 0) && (
        <details className="group bg-bg-surface border border-status-danger/25 rounded-xl">
          <summary className="flex items-center justify-between px-6 py-4 cursor-pointer list-none">
            <div className="flex items-center gap-3">
              <span className="text-status-danger text-sm font-medium">
                Failed items ({failedRuns.length + errorDocs.length})
              </span>
              <span className="text-text-muted text-xs">runs + corpus errors</span>
            </div>
            <span className="text-text-muted text-xs group-open:rotate-180 transition-transform">▼</span>
          </summary>

          <div className="px-6 pb-6 space-y-4 border-t border-status-danger/18 pt-4">
            {failedRuns.length > 0 && (
              <div>
                <p className="text-text-muted text-xs uppercase mb-2">Failed Runs</p>
                <div className="space-y-2">
                  {failedRuns.map((run: { id: string; created_at: string; study_id: string; error_message: string; phase: string }) => (
                    <div key={run.id} className="flex items-start justify-between text-xs py-2 border-b border-border-subtle last:border-0">
                      <div className="space-y-0.5">
                        <span className="text-text-muted font-mono">{run.id.slice(0, 8)} · {run.phase ?? "unknown"}</span>
                        {run.error_message && (
                          <p className="text-status-danger max-w-lg truncate">{run.error_message}</p>
                        )}
                      </div>
                      <span className="text-text-muted flex-shrink-0 ml-4">{new Date(run.created_at).toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              </div>
            )}

            {errorDocs.length > 0 && (
              <div>
                <p className="text-text-muted text-xs uppercase mb-2">Failed Corpus Ingestions</p>
                <div className="space-y-2">
                  {errorDocs.map((doc: { id: string; title: string; created_at: string }) => (
                    <div key={doc.id} className="flex items-center justify-between text-xs py-2 border-b border-border-subtle last:border-0">
                      <span className="text-text-muted">{doc.title}</span>
                      <span className="text-text-muted flex-shrink-0 ml-4">{new Date(doc.created_at).toLocaleDateString()}</span>
                    </div>
                  ))}
                </div>
              </div>
            )}
          </div>
        </details>
      )}
    </div>
  );
}
