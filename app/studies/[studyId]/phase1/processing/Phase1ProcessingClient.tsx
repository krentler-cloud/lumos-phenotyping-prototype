...
function buildStepDescriptions(
  ...
  corpusChunkCount: number,
  bySourceType: Record<string, number>
): Record<string, { what: string; why: string }> {
  const clinicalTrialDocs = bySourceType["clinical_trial"] ?? 0;
  const regulatoryDocs = bySourceType["regulatory"] ?? 0;
  const literatureDocs = bySourceType["literature"] ?? 0;
...
export default function Phase1ProcessingClient({
  studyId,
  corpusDocCount,
  corpusChunkCount,
  bySourceType,
  ...
  const STEP_DESCRIPTIONS = buildStepDescriptions(corpusDocCount, corpusChunkCount, bySourceType);
